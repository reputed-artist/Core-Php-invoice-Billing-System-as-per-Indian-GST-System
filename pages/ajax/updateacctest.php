...
if($try ==3 && $tp!=0 )
{
 ...
 $c=mysqli_query($con,"update `account` set  `cid` = $ds,`acc_type` = $tp,`opening_bal`=$opbal where `aid` = $aid") or die("Error: " . mysqli_error($con));

//echo "update `account` set  `cid` = $ds,`acc_type` = $tp,`opening_bal`=$opbal where `aid` = $aid and acc_type=1";
