<?php 
    ...
    if(!$conn){
        ...
        if($type == 'supplier')
        {
        $sql ='select pcid,pcname from purchasecom WHERE NOT EXISTS ( SELECT 1 FROM account WHERE account.cid = purchasecom.pcid )';

        }
        else{
            $sql='select cid,c_name from client WHERE NOT EXISTS ( SELECT 1 FROM account WHERE account.cid = client.cid )';
        }
